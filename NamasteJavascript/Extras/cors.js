...
// How it works -> When a site requests resource on another domain, first a CORS preflight mechanism is followed. CORS preflight options request is made to authorize using additional http headers if the site can request for resource to a particular server on different domain. Once authorized the server adds some other additional http headers to let the client know that this request is safe and then the actual call is made fo resource sharing.
...
// For some requests browsers flags it as simple call which does not require CORS preflight call but for others it is required.

// When wee try to request for resource from another domain, we oftne get CORS error, which is not actually an error, instead it is a standard that browsers have.

// It can be solved by adding headers to server ot to our request or start chrome with disable web security flag (not recommended) or installing some plugins which disable the CORS check (also not recommended).

// Instead of bypassing CORS, we c=should follow the CORS standard to improve security on web apps.
